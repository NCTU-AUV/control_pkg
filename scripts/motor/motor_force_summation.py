...
class Motor_Listener:
    def __init__(self):
        ...
        #self.coe = [1.11, -1.01, 1.42, -1.21, 1.50, -1] #motor 1 3 5 reverse   
        self.coe = [1, -1, 1, -1, 1, -1]
    ...
    def talker(self):
        for i in range(6):
            self.motor[i] = self.motor_attitude[i] + self.motor_depth[i] + self.motor_command[i] + self.motor_dc[i]
            self.motor[i] *= self.coe[i]
            #self.motor[i] *= 0.3
        print([f'{val:.2f}' for val in self.motor])
